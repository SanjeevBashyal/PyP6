...
def build_wbs_cache(cursor, proj_id):
    """
    Queries all WBS elements for a given project and returns a dictionary
    mapping their descriptive WBS Name to their internal wbs_id.
    This is highly efficient as it avoids querying the DB for every activity.
    """
...
def parse_relationship(relationship_str):
    ...
    # The database expects the format 'PR_FS', 'PR_SS', etc.
...
def main():
    ...
    try:
        ...
        proj_id, root_wbs_id, clndr_id, project_obs_id = get_project_defaults(cursor, cfg.TARGET_PROJECT_ID)
        wbs_name_cache = build_wbs_cache(cursor, proj_id)  # NEW: Build WBS cache
        ...
        for index, row in df.iterrows():
            ...
            # MODIFIED: Get WBS ID from the cache
            ...
            if task_code in activity_id_to_task_id:
                continue
            print(
                f"Processing Activity: {task_code} - {row['Activity_Name']} (under WBS: '{wbs_name}')"
            )
            cursor.execute(
            ...
            if cursor.fetchone():
                print(
                    f"  -> WARNING: Activity with code '{task_code}' already exists. Skipping insertion but mapping for relationships."
                )
                cursor.execute(
                    "SELECT task_id FROM TASK WHERE proj_id = ? AND task_code = ?",
                    (proj_id, task_code),
        ...
        # This part remains the same as it relies on the activity_id_to_task_id map, which is correctly populated.
        ...
        for index, row in df.iterrows():
            ...
            if not predecessors_str or predecessors_str.lower() == "nan":
                continue
